...
export default function CompleteProfile() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 1) Actualizar perfil + peso_inicial en Mongo
      const profilePayload = {
        fechaNacimiento: formData.fechaNacimiento
          ? formData.fechaNacimiento.split("-").reverse().join("/")   // yyyy-mm-dd → dd/mm/yyyy
          : "",
      ...
      // 2) Registrar primera medida corporal
      const bodyPayload = {
        peso:           parseFloat(formData.peso),
        ...
        pecho:          formData.pecho          ? parseFloat(formData.pecho)          : undefined,
        brazo_derecho:  formData.brazoDerecho   ? parseFloat(formData.brazoDerecho)   : undefined,
        brazo_izquierdo:formData.brazoIzquierdo ? parseFloat(formData.brazoIzquierdo) : undefined,
        muslo_derecho:  formData.musloDerecho   ? parseFloat(formData.musloDerecho)   : undefined,
        muslo_izquierdo:formData.musloIzquierdo ? parseFloat(formData.musloIzquierdo) : undefined,
        pantorrilla:    formData.pantorrilla    ? parseFloat(formData.pantorrilla)    : undefined,
        notas:          formData.notas || "",
      };
      await fetch("/api/user/body-progress", {
        method: "POST",
        headers,
        body: JSON.stringify(bodyPayload),
      });

      // 3) Marcar en localStorage que el perfil ya está completo
      //    (evita el redirect en próximos logins antes de que llegue del backend)
      try {
        const stored = JSON.parse(localStorage.getItem("user") || "{}");
        stored.peso_inicial = parseFloat(formData.peso);
