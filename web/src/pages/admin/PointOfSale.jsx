...
const CAT_COLORS = {
  Suplementos: "var(--accent)",
  Accesorios:  "var(--success)",
  Snacks:      "var(--warning)",
  Bebidas:     "#38bdf8",
};
    ...
    <>
      ...
      <header style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <FiShoppingCart size={22} color="var(--accent)" />
          <div>
      ...
      <div style={{ display: "flex", flex: 1, gap: 20, padding: "20px 28px", overflow: "hidden", height: "calc(100% - 73px)" }}>
            ...
            {CATEGORIAS.map(cat => (
              <button
                ...
                style={{
                  ...
                  background: categoria === cat ? "var(--accent)" : "var(--bg-card)",
                  color: categoria === cat ? "#fff" : "var(--text-secondary)",
                  border: `1px solid ${categoria === cat ? "transparent" : "var(--border)"}`,
                }}
              >
                {cat}
              </button>
            ...
            {productos.map(p => (
              <div
                ...
                style={{
                  ...
                  borderTop: `3px solid ${CAT_COLORS[p.categoria] || "var(--accent)"}`,
                }}
              ...
              >
                <div style={{ width: 36, height: 36, borderRadius: 8, background: `${CAT_COLORS[p.categoria] || "var(--accent)"}22`, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 10 }}>
                  <FiPackage size={18} color={CAT_COLORS[p.categoria] || "var(--accent)"} />
                </div>
                <span style={{ fontSize: 11, color: CAT_COLORS[p.categoria] || "var(--accent)", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".05em" }}>
                  {p.categoria}
                </span>
                <p style={{ fontSize: 14, fontWeight: 600, color: "var(--text-primary)", margin: "6px 0 10px", lineHeight: 1.3 }}>
                  {p.nombre}
                </p>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span style={{ fontSize: 18, fontWeight: 800, color: "var(--success)" }}>{fmt(p.precio)}</span>
        ...
        <div style={{ ...card({ padding: "20px", width: 300, flexShrink: 0, display: "flex", flexDirection: "column" }) }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, flexShrink: 0 }}>
            <span style={{ fontSize: 15, fontWeight: 700, color: "var(--text-primary)" }}>Carrito</span>
            {cart.length > 0 && (
              <button
                onClick={() => setCart([])}
          ...
          <div style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column", gap: 8, marginBottom: 14 }}>
            {cart.length === 0 ? (
              <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", color: "var(--text-secondary)", gap: 12, paddingTop: 40 }}>
              ...
              cart.map(item => (
                <div key={item.id} style={{ background: "var(--bg-input, rgba(255,255,255,.05))", borderRadius: 8, padding: "10px 12px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                    <span style={{ fontSize: 13, fontWeight: 600, color: "var(--text-primary)", flex: 1, lineHeight: 1.3 }}>{item.nombre}</span>
                    <button onClick={() => setQty(item.id, -item.qty)} style={{ background: "none", border: "none", color: "var(--danger)", cursor: "pointer", flexShrink: 0, marginLeft: 6 }}>
                      <FiTrash2 size={14} />
                    </button>
                  </div>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <button onClick={() => setQty(item.id, -1)} style={{ width: 24, height: 24, borderRadius: 6, border: "1px solid var(--border)", background: "none", color: "var(--text-primary)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                        <FiMinus size={12} />
                      </button>
                    ...
                    <span style={{ fontSize: 14, fontWeight: 700, color: "var(--success)" }}>{fmt(item.precio * item.qty)}</span>
                  </div>
                </div>
              ))
            )}
          ...
          {/* Error */}
          {error && (
            <div style={{ display: "flex", alignItems: "center", gap: 8, background: "rgba(239,68,68,.12)", border: "1px solid rgba(239,68,68,.3)", borderRadius: 8, padding: "8px 12px", marginBottom: 10, color: "var(--danger)", fontSize: 12 }}>
          ...
          <div style={{ borderTop: "1px solid var(--border)", paddingTop: 14, flexShrink: 0 }}>
            ...
            <div style={{ marginBottom: 12 }}>
              <label style={{ fontSize: 11, color: "var(--text-secondary)", display: "block", marginBottom: 6, textTransform: "uppercase", letterSpacing: ".06em" }}>
                Método de pago
              </label>
                ...
                {["Efectivo", "Tarjeta"].map(m => (
                  <button
                    ...
                    style={{
                      ...
                      background: metodo === m ? "var(--accent)" : "var(--bg-input, rgba(255,255,255,.05))",
            ...
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 14 }}>
              ...
              <span style={{ fontSize: 22, fontWeight: 800, color: "var(--text-primary)" }}>{fmt(total)}</span>
            </div>
            ...
            <button
              ...
              style={{
                ...
                background: vendido ? "var(--success)" : loading ? "#4f46e5" : cart.length ? "var(--accent)" : "var(--bg-input, rgba(255,255,255,.05))",
                color: cart.length || vendido || loading ? "#fff" : "var(--text-secondary)",
