...
export default function TrainerRoutines() {
  ...
  const loadRoutines = useCallback(async () => {
    try {
      ...
      const msg = err.message || "Error al cargar rutinas";
      setError(msg);
  ...
  /* ── Acciones ── */
  const handleDuplicate = async (e, id, routineName) => {
    e.stopPropagation();
    try {
      setActionLoading(true);
      await trainerService.duplicateRoutine(id);
      toast.success("Rutina duplicada", `Se creó una copia de "${routineName}".`);
      await loadRoutines();
    } catch (err) {
  ...
  const handleDelete = async (e, id, routineName) => {
    e.stopPropagation();

    const ok = await confirm({
      title:       "¿Eliminar rutina?",
      message:     `"${routineName}" se eliminará permanentemente y no podrá recuperarse.`,
      type:        "danger",
    ...
    try {
      ...
      toast.success("Rutina eliminada", `"${routineName}" fue eliminada correctamente.`);
      await loadRoutines();
    } catch (err) {
  ...
  const handleSave = async () => {
    if (!formData.name.trim()) {
      toast.warning("Campo requerido", "El nombre de la rutina es obligatorio.");
    ...
    try {
      ...
      if (editingId) {
        ...
        toast.success("Rutina actualizada", `"${formData.name}" fue guardada correctamente.`);
      } else {
        await trainerService.createRoutine(formData);
        toast.success("Rutina creada", `"${formData.name}" fue añadida a tu biblioteca.`);
      }
      setShowForm(false);
      await loadRoutines();
    } catch (err) {
  ...
  const addExercise = (di) =>
    setFormData((f) => {
      const days = [...f.days];
      days[di] = { ...days[di], exercises: [...days[di].exercises, { name: "", sets: "3", reps: "12", peso: "", notes: "" }] };
      return { ...f, days };
    });
      ...
      >
        <div style={{ display: "flex", gap: 15, alignItems: "center", flexWrap: "wrap" }}>
          <div className="input-dark-container with-icon" style={{ flex: 1, minWidth: 250 }}>
            ...
            <input
              type="text" className="search-input" placeholder="Buscar rutina…"
          ...
          <motion.button
            className="btn-compact-primary" onClick={openCreate}
      ...
      >
              ...
              >
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 15 }}>
                  <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                    <div style={{
                      width: 50, height: 50, background: "var(--bg-card)", borderRadius: 10,
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 15 }}>
                  {[
                    ...
                    <div key={s.label} style={{ background: "var(--bg-card)", padding: 8, borderRadius: 8, textAlign: "center" }}>
                      <div style={{ color: "var(--text-secondary)", fontSize: 11, marginBottom: 4 }}>{s.label}</div>
                ...
                <div style={{
                  ...
                  <div style={{ display: "flex", gap: 6 }}>
                    ...
                    <motion.button
                      className="icon-btn danger" style={{ padding: 6 }}
        ...
        {!loading && routines.length === 0 && (
          <div className="empty-state">
            ...
            <motion.button
              className="btn-compact-primary" style={{ marginTop: 16 }}
              onClick={openCreate}
              whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
            >
            ...
            >
              ...
              <div style={{
                ...
                <div style={{ display: "flex", gap: 15, alignItems: "center", flex: 1 }}>
                  <div style={{
                    width: 60, height: 60, background: "var(--bg-input)", borderRadius: 12,
                    display: "flex", alignItems: "center", justifyContent: "center",
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {(selectedRoutine.exerciseList || []).map((ex, idx) => (
                    <motion.div
                      key={idx}
                      style={{
                        background: "var(--bg-input)", padding: 15, borderRadius: 8,
        ...
        {showForm && (
          ...
          >
            ...
            >
              {/* Header form */}
              <div style={{
                padding: "20px 25px", borderBottom: "1px solid var(--border)",
                display: "flex", justifyContent: "space-between", alignItems: "center",
              }}>
              ...
              <div style={{ padding: 25 }}>
                ...
                <div style={{ marginBottom: 16 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                    ...
                    <motion.button className="btn-compact-primary" onClick={addDay} style={{ fontSize: 13 }}
                      whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                  ...
                  {formData.days.map((day, di) => (
                    ...
                    >
                      ...
                      >
                        <select
                          className="input-compact" style={{ width: "auto", flex: 1 }}
                          value={day.day}
                        ...
                        <input
                          className="input-compact" style={{ flex: 2 }}
                        ...
                        <motion.button
                          className="icon-btn danger" style={{ padding: 4 }}
                ...
                <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
                  ...
                  <motion.button
                    className="btn-compact-primary" onClick={handleSave} disabled={actionLoading}
