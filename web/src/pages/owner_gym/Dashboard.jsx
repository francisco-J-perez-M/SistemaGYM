...
function KpiCard({ label, value, valueColor, meta, badgeText, badgeType, progress, top }) {
  return (
    <div style={{ ...card(), borderTop: `3px solid ${top || "var(--accent, var(--accent))"}` }}>
      ...
      <div style={{ fontSize: 30, fontWeight: 800, color: valueColor || "var(--text-primary)", letterSpacing: "-.02em", margin: "4px 0" }}>
        {value}
      </div>
      {meta && <p style={{ fontSize: 12, color: "var(--text-secondary)", marginTop: 2 }}>{meta}</p>}
      {progress !== undefined && (
        <>
          <div style={{ height: 5, background: "var(--border)", borderRadius: 99, overflow: "hidden", marginTop: 10 }}>
            <div style={{ width: `${Math.min(progress, 100)}%`, height: "100%", background: top || "var(--accent, var(--accent))", borderRadius: 99, transition: "width 1s ease" }} />
          </div>
          <span style={{ fontSize: 11, color: "var(--text-secondary)", float: "right", marginTop: 3 }}>{progress}%</span>
        </>
      )}
    </div>
  );
...
function RetentionCard({ retention, churn }) {
  ...
  return (
    <div style={{ ...card(), borderTop: "3px solid var(--warning)" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
        <span style={{ fontSize: 12, fontWeight: 600, color: "var(--text-secondary)", textTransform: "uppercase", letterSpacing: ".06em" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 20, marginTop: 10 }}>
        <div style={{ position: "relative", width: 88, height: 88, flexShrink: 0 }}>
          <svg width="88" height="88" viewBox="0 0 88 88">
            <circle cx="44" cy="44" r={R} fill="none" stroke="var(--border)" strokeWidth="9" />
            <circle cx="44" cy="44" r={R} fill="none" stroke="var(--accent)" strokeWidth="9"
              strokeDasharray={`${filled} ${C}`} strokeDashoffset={C / 4} strokeLinecap="round"
              style={{ transition: "stroke-dasharray 1.2s ease" }} />
          </svg>
          <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 17, fontWeight: 800, color: "var(--text-primary)" }}>
            {retention}%
          </div>
        </div>
...
function BarChart({ revenues, expenses, labels }) {
  ...
  return (
    <div style={card({ flex: "1 1 0" })}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", paddingTop: 12, borderTop: "1px solid var(--border)", marginTop: 8 }}>
        <div>
          <p style={{ fontSize: 11, color: "var(--text-secondary)", textTransform: "uppercase", letterSpacing: ".05em", marginBottom: 2 }}>Promedio mensual</p>
          <p style={{ fontSize: 17, fontWeight: 700, color: "var(--accent-soft)" }}>{fmtM(avg)}</p>
        </div>
        <div style={{ textAlign: "right" }}>
          <p style={{ fontSize: 11, color: "var(--text-secondary)", textTransform: "uppercase", letterSpacing: ".05em", marginBottom: 2 }}>Mejor mes</p>
          <p style={{ fontSize: 17, fontWeight: 700, color: "var(--success)" }}>{fmtM(best)}</p>
        </div>
      </div>
    </div>
  );
...
function PeakChart({ data }) {
  ...
  return (
    <div style={card({ width: 280, flexShrink: 0 })}>
      <span style={{ fontSize: 14, fontWeight: 700, color: "var(--text-primary)", display: "block", marginBottom: 18 }}>Horas pico del día</span>
        ...
        {data.map((h, i) => {
          ...
          return (
            <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-end", height: "100%" }}>
              <div style={{ width: "100%", height: `${pct}%`, background: isPeak ? "var(--accent)" : "#334155", borderRadius: "3px 3px 0 0", minHeight: 2, opacity: isPeak ? 1 : .55 }} />
            </div>
          );
        })}
      </div>
      <div style={{ display: "flex", gap: 4, marginTop: 4 }}>
        {labels.map((l, i) => (
          <div key={i} style={{ flex: 1, textAlign: "center", fontSize: 9, color: "var(--text-secondary)" }}>{l}</div>
        ))}
      </div>
      <div style={{ marginTop: 12, padding: "8px 12px", background: "var(--bg-input, rgba(255,255,255,.05))", borderRadius: 8, fontSize: 12, color: "var(--text-secondary)" }}>
        Pico máximo: <strong style={{ color: "var(--text-primary)" }}>
        ...
        <thead>
          <tr style={{ borderBottom: "1px solid var(--border)" }}>
            {["Miembro", "Plan", "Vencimiento", "Estado"].map(h => (
              <th key={h} style={{ padding: "8px 12px", textAlign: "left", fontSize: 11, color: "var(--text-secondary)", textTransform: "uppercase", letterSpacing: ".06em", fontWeight: 600 }}>{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {rows.map((m, i) => (
            <tr key={m.id || i} style={{ borderBottom: "1px solid var(--border, rgba(255,255,255,.05))" }}>
              <td style={{ padding: "10px 12px", fontWeight: 600, color: "var(--text-primary)" }}>{m.miembro}</td>
              <td style={{ padding: "10px 12px", color: "var(--text-secondary)" }}>{m.plan}</td>
              <td style={{ padding: "10px 12px", color: "var(--text-secondary)" }}>{m.fecha_fin}</td>
  ...
  return (
    <>
      ...
      <header style={{
        ...
        background: "var(--bg-input)", flexShrink: 0, position: "sticky", top: 0, zIndex: 50,
      }}>
        <div>
          <h1 style={{ fontSize: 20, fontWeight: 700, color: "var(--text-primary)", margin: 0 }}>Panel Administrativo</h1>
          <p style={{ fontSize: 12, color: "var(--text-secondary)", margin: "2px 0 0" }}>Resumen en tiempo real</p>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          <span style={{ fontSize: 13, color: "var(--text-secondary)", padding: "5px 12px", border: "1px solid var(--border)", borderRadius: 8 }}>
            {dateStr}
          </span>
          <div style={{ width: 36, height: 36, borderRadius: "50%", background: "linear-gradient(135deg,var(--accent),var(--accent-soft))", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: "#fff" }}>
            {initials}
          </div>
        </div>
      </header>
