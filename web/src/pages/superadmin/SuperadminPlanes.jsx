...
function PlanForm({ plan, onSave, onCancel }) {
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div>
        <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "var(--text-secondary)", marginBottom: 6 }}>Máximo de miembros (vacío = ilimitado)</label>
...
function PlanCard({ plan, onEdit, onToggle }) {
  ...
  return (
    <div style={{ ...card(), borderTop: `3px solid ${plan.activo ? "var(--accent, var(--accent))" : "rgba(100,116,139,.4)"}`, display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <div>
        <span style={{ fontSize: 28, fontWeight: 800, color: "var(--text-primary)" }}>${precio}</span>
      ...
      {plan.suscriptores_activos != null && (
        <p style={{ fontSize: 12, color: "var(--text-secondary)", borderTop: "1px solid var(--border)", paddingTop: 10 }}>
...
export default function SuperadminPlanes() {
  ...
  const handleToggle = async (plan) => {
    const { isConfirmed } = await Swal.fire({
      ...
      confirmButtonColor: plan.activo ? "var(--danger)" : "var(--success)",
    ...
    try {
      ...
      if (r.data.advertencia) {
        Swal.fire({ icon: "warning", title: "Plan desactivado", text: r.data.advertencia, background: "var(--bg-card)", color: "var(--text-primary)" });
      } else {
        Swal.fire({ icon: "success", title: "Listo", timer: 1500, showConfirmButton: false, background: "var(--bg-card)", color: "var(--text-primary)" });
      }
      load();
    } catch (e) {
  ...
  const handleSave = ()     => { setShowForm(false); setEditing(null); load();
    Swal.fire({ icon: "success", title: "Plan guardado", timer: 1500, showConfirmButton: false, background: "var(--bg-card)", color: "var(--text-primary)" }); };

  return (
    <div style={{ padding: "28px 32px", minHeight: "100vh", background: "var(--bg-input)", fontFamily: "inherit" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 28 }}>
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 800, color: "var(--text-primary)", marginBottom: 4 }}>Planes de Suscripción</h1>
          <p style={{ fontSize: 14, color: "var(--text-secondary)" }}>Gestiona los planes disponibles para los gimnasios</p>
        </div>
      ...
      {loading ? (
        <div style={{ textAlign: "center", padding: 60, color: "var(--text-secondary)" }}>Cargando planes…</div>
      ) : (
